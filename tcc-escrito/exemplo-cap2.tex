...
\vspace{0.5em}
\noindent\textbf{Visualização 1 — Autopercepção do conhecimento em educação financeira (dados reais).}
...
\vspace{0.5em}
\noindent\textbf{Visualização 2 — Série histórica recente do I-SFB (dados reais).}
...
\noindent \textbf{Descrição:} O Mobills, criado no Brasil e expandido para o mercado norte-americano, oferece controle financeiro completo com dashboards personalizáveis, metas, gerenciamento de cartões, alertas de fatura e integração automática com bancos via open finance. A plataforma disponibiliza módulos premium com relatórios avançados, exportação de dados e acompanhamento de investimentos, além de comunidade ativa com conteúdos didáticos e trilhas de aprendizado.

% Substitua o caminho abaixo pela imagem real que você baixar
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.65\linewidth]{figs/mobills.jpg}
...
\subsection*{Tabela comparativa de funcionalidades}

\begin{table}[H]
\centering
\caption{Comparativo de funcionalidades entre ferramentas concorrentes.}
\label{tab:comparativo_ferramentas}
\begin{tabular}{lcccc}
\hline
\textbf{Feature} & \textbf{Guiabolso} & \textbf{Organizze} & \textbf{Minhas Economias} & \textbf{Mobills} \\
\hline
Integração bancária automática & Sim & Não & Não & Sim \\
Categorização de gastos    & Sim & Sim & Sim & Sim \\
Planejamento/metas    & Limitado & Sim & Sim & Sim \\
Recursos educativos    & Poucos & Não & Não & Poucos \\
Custos / Paywall    & Não & Não & Não & Sim \\
Design / UX atual    & Médio & Bom & Fraco & Bom \\
\hline
\end{tabular}
\end{table}

\paragraph{Síntese.}
As alternativas existentes evoluíram a experiência de controle financeiro, mas ainda deixam lacunas importantes. Observam-se três frentes críticas:
\begin{itemize}
    \item \textbf{Educação prática integrada ao fluxo}. Orientar decisões sem exigir buscas externas permanece como diferencial desejado;
    \item \textbf{Análises visuais acionáveis}. É preciso oferecer narrativas guiadas que expliquem causas e recomendações em linguagem simples;
    \item \textbf{Acessibilidade}. Funcionalidades essenciais devem estar disponíveis sem barreiras financeiras ou técnicas.
\end{itemize}
O \textbf{Dashboard Financeiro} combina essas frentes para promover entendimento, engajamento e mudança de comportamento, alinhando-se às evidências sobre o estado da educação financeira no país \cite{FEBRABAN2025,ANBIMA2024}.
