...
\sigla{BI}{Business Intelligence}
\sigla{C4}{Context, Containers, Components and Code (modelo de arquitetura)}
\sigla{CEP}{Código de Endereçamento Postal}
\sigla{CPF}{Cadastro de Pessoas Físicas}
\sigla{CNAME}{Canonical Name record}
\sigla{CORS}{Cross-Origin Resource Sharing}
\sigla{CRUD}{Create, Read, Update, Delete}
\sigla{DKIM}{DomainKeys Identified Mail}
\sigla{DNS}{Domain Name System}
\sigla{FEBRABAN}{Federação Brasileira de Bancos}
...
\sigla{I-SFB}{Índice de Saúde Financeira do Brasileiro}
\sigla{JPA}{Java Persistence API}
\sigla{JSON}{JavaScript Object Notation}
\sigla{JWT}{JSON Web Token}
\sigla{MUI}{Material UI}
\sigla{MVP}{Minimum Viable Product}
\sigla{NPS}{Net Promoter Score}
\sigla{OTP}{One-Time Password}
\sigla{OWASP}{Open Worldwide Application Security Project}
\sigla{REST}{Representational State Transfer}
\sigla{SPF}{Sender Policy Framework}
...
\abrev{p.p.}{pontos percentuais}
\abrev{TC I}{Trabalho de Conclusão I}
\abrev{TC II}{Trabalho de Conclusão II}

\simbolo{R\$}{Real brasileiro}
\simbolo{\%}{Percentual}
...
Todos os objetivos foram endereçados no volume atual: o Capítulo~\ref{chap:chap3} consolida a arquitetura orientada a serviços, o Capítulo~\ref{chap:chap4} descreve a infraestrutura em produção e documenta a rastreabilidade das entregas, e o Capítulo~\ref{chap:chap5} detalha a validação funcional e os próximos passos.
